...
// Seeded RNG (mulberry32) — module-scope data must be identical on server and
// client or React hydration breaks and dashboard counts drift between renders.
let _seed = 1437; // Hijri year of the first Saudi smart-Hajj pilot — any constant works
...
function buildAlertHistory(pilgrimSeed: number): AlertHistoryEntry[] {
  const count = Math.floor(random() * 5); // 0–4 past alerts
  return Array.from({ length: count }, (_, k) => {
    const level: RiskLevel = random() > 0.6 ? "red" : "yellow";
    const resolved = k > 0 || random() > 0.3; // most past ones resolved
    return {
...
export const MOCK_PILGRIMS: MockPilgrim[] = Array.from({ length: 40 }, (_, i) => {
  ...
  const hasDiabetes = random() > 0.7;
  const hasHeartCondition = random() > 0.85;
  const hasHypertension = random() > 0.65;
  const meds: string[] = [];
  if (hasHypertension) meds.push("أملوديبين ٥مغ");
  if (hasDiabetes) meds.push(random() > 0.5 ? "ميتفورمين ٥٠٠مغ" : "إنسولين");
  if (hasHeartCondition) meds.push("أسبرين ٧٥مغ");
  if (meds.length === 0 && random() > 0.7) meds.push(pick(MEDS));
  ...
  return {
    ...
    condition: random() > 0.6 ? pick(CONDITIONS) : undefined,
    alertTime: random() > 0.5 ? pick(TIMES) + " ago" : undefined,
