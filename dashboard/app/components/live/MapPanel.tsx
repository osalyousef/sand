...
const LeafletMap = dynamic(() => import("./LeafletMap"), {
  ssr: false,
  loading: () => (
    <div className="flex-1 flex items-center justify-center bg-[#0d1117] text-gray-600 text-sm">
      جارٍ تحميل الخريطة…
    </div>
  ),
});
    ...
    <div className="flex-1 flex flex-col bg-gray-900 rounded-xl border border-gray-800 overflow-hidden">
        ...
        <div className="flex items-center bg-gray-800 rounded-lg p-0.5 gap-0.5">
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-1 text-xs rounded-md transition-all ${
              mode === "now" ? "bg-emerald-600 text-white" : "text-gray-400 hover:text-white"
            }`}
          >
      ...
      <div className="relative flex-1 overflow-hidden">
        ...
        <div className="absolute top-3 right-3 z-[1000] pointer-events-none">
          ...
          >
            {mode === "now" ? <Zap className="w-3.5 h-3.5" /> : <TrendingUp className="w-3.5 h-3.5" />}
            {mode === "now"
              ? "كثافة الحالات الحرجة الآن"
              : "المناطق الحرجة المتوقعة خلال ٤ ساعات"}
          </div>
        </div>

        {/* Density legend */}
        <div className="absolute top-3 left-3 z-[1000] flex flex-col gap-2 bg-gray-950/85 p-2.5 rounded-lg border border-gray-800 text-xs">
